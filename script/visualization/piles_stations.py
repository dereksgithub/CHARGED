...
"""
Script for visualizing charging stations and piles across multiple cities.

This script creates a comprehensive visualization showing the distribution
of electric vehicle charging stations and individual charging piles across
six major cities. It generates both a grid layout overview and detailed
inset views for specific areas of interest.

The visualization includes:
    - City boundary maps
    - Station locations (outlined circles)
    - Individual pile locations (dots)
...
if __name__ == '__main__':
    ...
    for idx, (city_cn, (addr, abr), spec) in enumerate(layout):
        ...
        data_dir = f'../../data/{abr}/'
        piles = pd.read_csv(os.path.join(data_dir, 'piles.csv'), dtype={'station_id': str})
        stations = pd.read_csv(os.path.join(data_dir, 'stations.csv'), dtype={'station_id': str})

        piles_gdf = gpd.GeoDataFrame(
            piles,
            geometry=gpd.points_from_xy(piles.longitude, piles.latitude),
            crs='EPSG:4326'
        ).to_crs(target_crs)
        ...
        stations_gdf.plot(ax=ax, marker='o', markersize=30,
                          facecolor='none', edgecolor='#004600', alpha=0.8, linewidth=1.2, label='Stations')
        piles_gdf.plot(ax=ax, marker='.', markersize=10, alpha=0.6, color='#DD7C4F', label='Piles')
        ...
        if idx == 0:
            top_station = piles['station_id'].value_counts().index[0]
            station_point = stations_gdf[stations_gdf['station_id'] == top_station]
            pile_points = piles_gdf[piles_gdf['station_id'] == top_station]

            x_buf = 2
            y_buf = 4
            xmin, ymin, xmax, ymax = pile_points.total_bounds
            ...
            pile_points.plot(ax=axins, marker='.', markersize=10, alpha=0.6, color='#DD7C4F', )
            # station_point.plot(ax=axins, marker='o', markersize=30,facecolor='none', edgecolor='#DD7C4F', linewidth=1.2)
            ...
            pile_points.plot(ax=ax2, marker='.', markersize=12, alpha=0.7, color='#1f77b4')
            plt.tight_layout()
            ax2.axis('off')
            inset_path = os.path.join(output_fig_dir, f'inset_station.png')

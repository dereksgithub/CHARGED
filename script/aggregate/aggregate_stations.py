...
"""
Script for clustering charging stations based on geographic proximity.

This script uses DBSCAN clustering algorithm to group charging stations
that are within a specified distance threshold. It processes station data
from multiple cities and generates cluster information including station IDs
...
def cluster_station(data, distance):
    """
    Cluster stations based on geographic proximity using DBSCAN.
    
    Args:
        data (pd.DataFrame): DataFrame containing station data with 'latitude', 
                           'longitude', and 'station_id' columns
    ...
    Returns:
        dict: Dictionary with cluster information where each key is a cluster ID
              and value is [station_ids, centroid_longitude, centroid_latitude]
    """
    ...
    # Aggregate station IDs and calculate sum of coordinates for centroid
...
if __name__ == '__main__':
    for city_cn,city_info in tqdm(city_dict.items(), desc="Processing city data"):
        ...
        station_cluster=cluster_station(data,r)
        result_file = os.path.join(data_path, 'station_cluster.json')
        with open(result_file, 'w', encoding='utf-8') as f:
            json.dump(station_cluster, f, ensure_ascii=False, indent=4)
